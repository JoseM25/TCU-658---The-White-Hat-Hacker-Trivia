class GameIconsMixin:
    ...
    def load_header_icons(self):
        self.clock_icon = self.load_icon("clock", 24)
        self.star_icon = self.load_icon("star", 24)
        self.freeze_icon = self.load_icon("freeze", 24)
    ...
    def load_delete_icon(self):
        self.delete_icon = self.load_icon("delete", self.BASE_SIZES["delete_icon_base"])

    def load_lightning_icon(self, size=18):
        self.lightning_icon = self.load_icon("lightning", size)

    def load_freeze_wildcard_icon(self, size=28):
        self.freeze_wildcard_icon = self.load_icon("freeze", size)
